			  ...
			  snapshot.forEach(function(data) {
				...
				if (issuecount == 0) {
					showresult("There are no open ticket");
				 	...
				 	showresult("There " + ((issuecount == 1) ? "is ":"are " )+ issuecount + " open ticket");
				}
			});
});
...
var displayIssue = function(key,priority,raisedBy,subject,dateraise,description,department,status,lastupdatedDate,assignto,assigneeName) {
		...
		getUsername(raisedBy,function(username) { 
	        ...
	        '<td> <button type="button" class="btn" style="font-size:12px" data-toggle="modal" data-target="#assign" ' +
	        'onclick=\'assign("'+key+ '")\' id="btn'+ key+'" >Assign</button> ' +
	        '<label id="lbl'+ key+'" >'+assigneeName+'</label>  </td>' +
	        '<td> <button type="button" class="btn"  style="font-size:12px;padding-top:-2px"  data-toggle="modal" data-target="#description" onclick=\'viewdescription("'+description+ '")\'>View Description</button> </td>' +
	        '<td> <button type="button" class="btn " style="font-size:12px" data-toggle="modal" data-target="#commenting" onclick=\'comment("'+key+ '")\'>comment</button> </td>' +
	        '<td> <button type="button" class="btn  " style="font-size:12px" onclick=\'closeissue("'+key+ '")\'>close issue</button> </td>' +
	        '</tr>';
